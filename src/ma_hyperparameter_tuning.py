"""
Hyperparameter Tuning Module for MA Optimizer using Optuna.

This module provides automated hyperparameter optimization for the
Memetic Algorithm optimizer using Optuna's TPE sampler.
...
Usage:
    python ma_hyperparameter_tuning.py --n_trials 50 --dataset 5Health-rand --model NCF
"""
...
class MAHyperparameterTuner:
    """
    Optuna-based hyperparameter tuner for the MA Optimizer.

    This class defines the search space, objective function, and manages
    the optimization study for finding optimal MA parameters.
    """
    ...
    def __init__(
        ...
        if os.path.exists(cache_file):
            ...
            self._prebuilt_data = MAOptimizer.load_vectorized_data(cache_file)
            self.logger.info(f"Loaded vectorized data from cache: {cache_file}")
        else:
            # Build and save to disk cache for future runs
            self._prebuilt_data = MAOptimizer.build_vectorized_data(data_loader, k)
            MAOptimizer.save_vectorized_data(self._prebuilt_data, cache_file)
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            # Create MA optimizer with trial parameters and pre-built data
            ma = MAOptimizer(
    ...
    def run_study(
        ...
        """
        study_name = study_name or f"ma_tuning_{self.model_name}_{self.group_name}"
    ...
    def generate_optimized_config(self, results: Dict) -> str:
        ...
        config_code = f"""# Optimized MA Parameters (generated by hyperparameter tuning)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Hyperparameter tuning for MA Optimizer using Optuna"
    )
    parser.add_argument(
        "--n_trials",
        type=int,
