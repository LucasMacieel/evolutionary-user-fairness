...
The optimization balances quality (50%) and speed (50%), finding
hyperparameters that achieve good recommendation quality while
...
class GAHyperparameterTuner:
    ...
    def __init__(
        ...
        max_generations: int = 50,  # Safety ceiling for GA (early stopping halts earlier)
        seed: int = 42,
        ...
        Optimization balances quality (50%) and speed (50%), finding
        configurations that achieve good results quickly.
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        Optimizes for a combined score of 50% quality (sum of preference scores)
        and 50% speed (inverse of CPU time). This balances finding high-quality
        solutions with computational efficiency.

        Args:
            trial: Optuna trial object

        Returns:
            Combined score to maximize: 0.5 * normalized_quality + 0.5 * speed_score
        """
        ...
        try:
            ...
            # Compute combined objective: 50% quality + 50% speed
            # Normalize quality by dividing by 10000 (typical preference score magnitude)
            normalized_quality = best_fitness / 10000.0
            ...
            combined_score = 0.5 * normalized_quality + 0.5 * speed_score

            trial.set_user_attr("combined_score", float(combined_score))
            trial.set_user_attr("normalized_quality", float(normalized_quality))
            trial.set_user_attr("speed_score", float(speed_score))

            self.logger.info(
                f"Trial {trial.number}: combined_score={combined_score:.4f} "
                f"(quality={normalized_quality:.4f}, speed={speed_score:.4f})"
            )
    ...
    def run_study(
        ...
        for param, value in best_params.items():
            print(f"  {param}: {value}")
        print(f"\nBest Objective (50% quality + 50% speed): {best_value:.4f}")
    ...
    def generate_optimized_config(self, results: Dict) -> str:
        ...
        # Best Objective (combined score): {results["best_score"]:.4f}
...
def main():
    ...
    # Create tuner (optimizes for 50% quality + 50% speed)
